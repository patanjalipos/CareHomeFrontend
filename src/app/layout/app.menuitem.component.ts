...
@Component({
    ...
    template: `
		<ng-container>
			...
			<a title="{{item.title}}" *ngIf="(!item.routerLink || item.items) && item.visible !== false" [attr.href]="item.url" (click)="itemClick($event)"
			...
			<a title="{{item.title}}" *ngIf="(item.routerLink && !item.items) && item.visible !== false" (click)="itemClick($event)" [ngClass]="item.class" 
			...
			<ul class="big-menu" *ngIf="item.items && item.visible !== false" [@children]="submenuAnimation">
